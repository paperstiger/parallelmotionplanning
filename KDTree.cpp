...
Real Distance(const Vector& a, const Vector& b, Real norm) {
    double result = 0;
    if(norm == 1) {
    ...
    for(size_t i = 0; i < a.size(); i++)
        result += pow(a[i] - b[i], norm);
    return pow(result, 1. / norm);
}

Real Distance_Weighted(const Vector& a,const Vector& b,Real norm,const Vector& weights) {
    double result = 0;
    if(norm == 1) {
        for(size_t i = 0; i < a.size(); i++)
    ...
    for(size_t i = 0; i < a.size(); i++)
        result += pow(a[i] - b[i], norm) * weights[i];
    return pow(result, 1. / norm);
}
...
inline int _MaxDist(Real* dist, int k)  // search along first k entries, the index with maximum value
{
  int imax=0;
  for(int i=1;i<k;i++)
...
bool KDTree::Split(int dimension)
{
  ...
  splitDim = dimension;
  if(dimension < 0)
    splitDim = RandInt(pts[0].pt.size());
...
KDTree* KDTree::Insert(const Vector& p,int id,int maxLeafPoints)
{
  ...
  if((int)node->pts.size() >= maxLeafPoints) {
    //split
    if(node->Split(node->depth % p.size()))
...
void KDTree::ClosePoints(const Vector& pt,Real radius,std::vector<Real>& distances,std::vector<int>& ids) const
{
  if(IsLeaf()) {
    Real r2 = Sqr(radius);
    for(size_t i=0;i<pts.size();i++) {
      Real d2=distanceSquared(pts[i].pt, pt);
      if(d2 < r2) {
